...
def e_step(experiment_data: List[NDArray[np.int_]],
    ...
    for draw in experiment_data:
        posterior1 = dice_posterior(draw, bag_of_dice) #keeps showing not defined error?
        posterior2 = 1 - posterior1

        expected_counts[0] += posterior1 * draw[:max_number_of_faces] 
        expected_counts[1] += posterior2 * draw[:max_number_of_faces] 
...
def m_step(expected_counts_by_die: NDArray[np.float_]):
    ...
    for i in range(len(expected_counts_by_die[0])):
    ...
    updated_bag_of_dice = BagOfDice(updated_priors,
                                    [Die(updated_type_1_face_probs.tolist()),
                                     Die(updated_type_2_face_probs.tolist())])
